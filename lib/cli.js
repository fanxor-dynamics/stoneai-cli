...
import { doctorStatus, recordFromArgs, renderJson, renderMarkdown, template, validateRecord, parseRecordFile } from './issue-record.js';
...
async function issue(args) {
  ...
  if (verb === 'template') return out(template().trimEnd());
  if (verb === 'render') {
    const record = renderJson(recordFromArgs(clean));
    const result = validateRecord(record);
    if (!result.success) throw issueValidationError(result);
    return out(json ? JSON.stringify(record, null, 2) : renderMarkdown(record).trimEnd());
  }
  if (verb === 'validate') {
    const file = clean.find((arg) => !arg.startsWith('-'));
    if (!file) throw new Error('usage: stoneai issue validate <file> [--json]');
    const result = validateRecord(parseRecordFile(file));
    if (json) out(JSON.stringify(result, null, 2));
    if (result.success) return json ? undefined : out(`  ${C('✓')} Issue Resolution Record valid`);
    throw issueValidationError(result);
  }
  if (verb === 'doctor') {
    const status = doctorStatus();
    if (json) return out(JSON.stringify(status, null, 2));
    out(`  ${C(status.success ? '✓' : '×')} issue record template: ${status.template}`);
    out(`  ${C(status.success ? '✓' : '×')} issue record schema: ${status.schema}`);
    return out(`  ${C('✓')} offline mode: no auth, network, or secrets required`);
  }
  throw new Error('usage: stoneai issue template|render|validate|doctor');
}
function issueValidationError(result) {
  return new Error(`missing required fields: ${result.missing.join(', ') || 'none'}; invalid fields: ${result.invalid.join(', ') || 'none'}`);
}


function issueHelp() {
  out(`
  ${G('StoneAI issue records')} — cradle-to-grave evidence packets

  ${G('stoneai')} issue template
          issue render --symptoms <s> --root-cause <r> --fix <f> --results <r> --preventative-measures <p> --future-outlook <f>
          issue render --json ...
          issue validate <file> [--json]
          issue doctor [--json]
`);
}
...
async function doctor(args = []) {
  ...
  let reachable = false, healthBody = null, netDetail = '';
  try {
    ...
    check('backend reachable', res.ok, netDetail);
  } catch (e) {
...
function help() {
  ...
  ${G('stoneai')} login              browser sign-in (or --with-key to paste)
          ...
          stoneai issue template     print an Issue Resolution Record skeleton
          issue validate <file> [--json]   verify required issue evidence
          doctor [--json]    diagnose CLI, network, TLS, auth, and local state
          config [set k v]   base_url etc.
          --version | --help
`);
}
function restIssue(args) {
  if (args.includes('--help') || args.includes('-h')) return issueHelp();
  return issue(args);
}
...
export async function run(argv) {
  ...
  switch (cmd) {
    ...
    case 'issue': return restIssue(args);
