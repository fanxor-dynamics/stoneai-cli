...
// ---- request-builder helpers (pure, no I/O) ----
// Exported so tests can pin the exact shapes sent to the server.

/** Body for POST /v1/handoffs */
export function buildHandoffCreateBody(fromSessionId, toSurface) {
  return { fromSessionId, fromSurface: 'cli', toSurface, stateSnapshot: {} };
}

/** Body for POST /v1/handoffs/:id/accept */
export function buildHandoffAcceptBody(toSessionId) {
  return { toSessionId };
}

/** Body for POST /v1/missions */
export function buildMissionCreateBody(title, opts) {
  const body = { title };
...
/** Body for POST /v1/missions/:id/checkpoint (singular path).
  * label is required; a timestamp default is supplied when omitted. */
export function buildCheckpointBody(label) {
  return { label: label || `checkpoint ${new Date().toISOString()}`, state: {} };
}

/** Query string for GET /v1/missions */
export function buildMissionsQuery(opts) {
...
/** Query string for GET /v1/artifacts */
export function buildArtifactsQuery(opts) {
  const qs = new URLSearchParams();
...
/** Resolve the calling device's current session ID from GET /v1/sessions.
 * Requires a session-authenticated token (not a bare API key).
 * Throws with an actionable message when no session can be found. */
async function resolveCurrentSessionId() {
  let list;
  try {
    list = await api('/v1/sessions');
  } catch (e) {
    throw new Error(
      `handoff requires a session — could not list sessions: ${e.message}\n` +
      `  If you are logged in with an API key only, run \`stoneai login\` to create a session.`,
    );
  }
  const rows = Array.isArray(list) ? list : (list?.sessions || []);
  ...
  if (!s?.id) {
    throw new Error(
      'no active session found — handoff requires a session-authenticated login.\n' +
      '  Run `stoneai login` to create a session.',
    );
  }
  return s.id;
}
async function handoff(args) {
  if (args[0] === 'accept') {
    ...
    if (!handoffId) throw new Error('usage: stoneai handoff accept <handoff-id>');
    const toSessionId = await resolveCurrentSessionId();
    const r = await api(`/v1/handoffs/${handoffId}/accept`, {
      method: 'POST',
      body: buildHandoffAcceptBody(toSessionId),
    });
  ...
  const fromSessionId = await resolveCurrentSessionId();
  const r = await api('/v1/handoffs', {
    method: 'POST',
    body: buildHandoffCreateBody(fromSessionId, surface),
  });
  const handoffId = r.handoffId || r.id || '????';
  ...
  out(row(`  From: ${from}`));
  out(row(`  To:   ${to}`));
  out(row(`  ID:   ${handoffId}`));
  out(row(''));
  out(row('  On target device, run:'));
  out(row(`  stoneai handoff accept ${handoffId}`));
  out(`  ╚${'═'.repeat(w)}╝`);
  out('');
}
async function projects(args) {
  ...
  if (!rows.length) return out(D('  no projects found'));
  out('');
  out('  Projects:');
  out('');
  for (const p of rows) {
    const wsLabel = p.workspace_id ? D(` · ws ${short(p.workspace_id)}`) : '';
    out(`  📂 ${(p.name || p.slug || p.id || 'unnamed').padEnd(28)}${wsLabel}`);
  }
}
async function missions(args = []) {
  ...
  const qs = new URLSearchParams();
  if (status) qs.set('status', status);
  if (workspace) qs.set('workspaceId', workspace);
...
async function mission(args = []) {
  if (args[0] === 'create') {
    ...
    if (workspace) body.workspaceId = workspace;
...
async function checkpoint(args = []) {
  ...
  const cp = await api(`/v1/missions/${missionId}/checkpoint`, { method: 'POST', body: buildCheckpointBody(label) });
...
async function devices(args) {
  ...
  for (const d of rows) {
    ...
    const name = (d.label || d.name || '—').padEnd(20);
