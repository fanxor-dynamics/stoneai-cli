...
// ---- Hugging Face Nexus ----
function hfAdminHeaders() {
  const token = process.env.STONEAI_ADMIN_TOKEN;
  if (!token) throw new Error('HF Nexus operator commands require STONEAI_ADMIN_TOKEN in the environment');
  return { 'x-admin-token': token };
}
function hfJson(args, data) {
  if (args.includes('--json')) return out(JSON.stringify(data, null, 2));
  return false;
}
async function hf(args = []) {
  const [verb = 'status', ...rest] = args;
  const headers = hfAdminHeaders();
  if (verb === 'status') {
    const d = await api('/v1/admin/hf-nexus', { headers });
    if (hfJson(rest, d) !== false) return;
    out(`  ${G('HF Nexus')} · ${d.total ?? 0} assets · last scan ${d.lastScanAt || 'never'}`);
    for (const [state, n] of Object.entries(d.states || {})) out(`  ${String(state).padEnd(12)} ${n}`);
    if (d.errors?.length) out(R(`  discovery errors: ${d.errors.length}`));
    return;
  }
  if (verb === 'scan') {
    const d = await api('/v1/admin/hf-nexus/scan', { method: 'POST', body: {}, headers });
    if (hfJson(rest, d) !== false) return;
    out(`  ${C('✓')} HF Radar scan complete · ${G(d.total ?? 0)} assets`);
    for (const [state, n] of Object.entries(d.states || {})) out(`  ${String(state).padEnd(12)} ${n}`);
    return;
  }
  if (verb === 'assets' || verb === 'models' || verb === 'approved' || verb === 'evaluating') {
    const qs = new URLSearchParams();
    if (verb === 'models') qs.set('kind', 'model');
    if (verb === 'approved') qs.set('state', 'APPROVED');
    if (verb === 'evaluating') qs.set('state', 'EVALUATING');
    const capability = rest.find((a) => a.startsWith('--capability='))?.split('=')[1];
    const state = rest.find((a) => a.startsWith('--state='))?.split('=')[1];
    const kind = rest.find((a) => a.startsWith('--kind='))?.split('=')[1];
    const limit = rest.find((a) => a.startsWith('--limit='))?.split('=')[1] || '100';
    if (capability) qs.set('capability', capability);
    if (state) qs.set('state', state.toUpperCase());
    if (kind) qs.set('kind', kind);
    qs.set('limit', limit);
    const d = await api(`/v1/admin/hf-nexus/assets?${qs}`, { headers });
    if (hfJson(rest, d) !== false) return;
    if (!d?.length) return out(D('  no matching HF Nexus assets'));
    for (const a of d) out(`  ${G(a.key)}  ${String(a.state || '').padEnd(11)} ${D((a.capabilities || []).join(','))}`);
    return;
  }
  if (verb === 'tevv') {
    const key = rest.find((a) => !a.startsWith('-'));
    if (!key) throw new Error('usage: stoneai hf tevv <asset-key>');
    const d = await api(`/v1/admin/hf-nexus/assets/${encodeURIComponent(key)}/tevv`, { headers });
    return out(JSON.stringify(d, null, 2));
  }
  if (verb === 'routes' || verb === 'tools') {
    const d = await api(`/v1/admin/hf-nexus/${verb}`, { headers });
    return out(JSON.stringify(d, null, 2));
  }
  if (verb === 'state') {
    const [key, state] = rest.filter((a) => !a.startsWith('-'));
    if (!key || !state) throw new Error('usage: stoneai hf state <asset-key> <QUARANTINED|EVALUATING|APPROVED|ACTIVE|DEPRECATED|REJECTED>');
    const allowed = ['QUARANTINED','EVALUATING','APPROVED','ACTIVE','DEPRECATED','REJECTED'];
    const next = state.toUpperCase();
    if (!allowed.includes(next)) throw new Error(`invalid HF Nexus state "${state}"`);
    const d = await api('/v1/admin/hf-nexus/assets/state', { method: 'POST', body: { key, state: next }, headers });
    if (hfJson(rest, d) !== false) return;
    out(`  ${C('✓')} ${G(key)} → ${next}`);
    return;
  }
  throw new Error('usage: stoneai hf status|scan|assets|models|evaluating|approved|tevv|routes|tools|state');
}
