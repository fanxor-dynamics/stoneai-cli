...
const spectrum = ['38;2;97;187;70','38;2;253;184;39','38;2;245;130;31','38;2;224;58;62','38;2;150;61;151','38;2;0;157;220'];
const paint = (s, code) => ANSI ? `\x1b[${code}m${s}\x1b[0m` : s;
function stoneBanner() {
  ...
  out('');
  rows.forEach((row, i) => out(paint(row, spectrum[i])));
  out('');
  out(`  ${G('STONEAI')} · OMNI INTELLIGENCE SYSTEM ${D('· governed by humanity')}`);
}
...
async function whoami() {
  ...
  const [s, account] = await Promise.all([api('/v1/stats'), api('/v1/account/context')]);
  out(`  ${G('StoneAI')} · ${load().base_url}`);
  const sub = account?.subscription || {};
  const usage = account?.usage || {};
  out(`  plan ${G(sub.plan || 'trial')} · subscription ${sub.active ? C('active') : D(sub.status || 'trial')} · decrees ${C(usage.decrees_used ?? 0)}/${usage.decrees_included ?? '—'}`);
...
export async function run(argv) {
  const [cmd, ...args] = argv;
  if (cmd === undefined && process.stdout.isTTY) stoneBanner();
