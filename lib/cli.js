...
async function plans() {
  ...
  for (const x of list) out(`  ${G((x.id || x.name).padEnd(10))} $${((x.baseCents ?? x.base ?? 0) / 100).toFixed(2)}/mo · ${x.included ?? x.includedDecrees ?? '—'} decrees`);
}
async function upgrade(args) {
  const plan = args.find((a) => !a.startsWith('-')) || 'micro';
