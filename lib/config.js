...
// stoneai.fanz.website was retired with the rest of *.fanz.website (ch.176/187
// brand migration) — wroteinstone.com is the live production host.
const DEFAULT_BASE = 'https://wroteinstone.com';

export function load() {
  if (!existsSync(FILE)) return { base_url: process.env.STONEAI_BASE_URL || DEFAULT_BASE };
  try {
    const cfg = JSON.parse(readFileSync(FILE, 'utf8'));
    return { base_url: DEFAULT_BASE, ...cfg, ...(process.env.STONEAI_BASE_URL ? { base_url: process.env.STONEAI_BASE_URL } : {}) };
  } catch {
    return { base_url: process.env.STONEAI_BASE_URL || DEFAULT_BASE };
  }
}
